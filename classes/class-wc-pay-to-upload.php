...
class WC_Pay_To_Upload extends Airy_Framework {
	...
	function __construct() {
		...
		$this->plugin_name = __('Pay to Upload', $this->shortname );
		$this->desc = __('Let WC customers pay to upload files when purchasing specific products.', $this->shortname );
		$this->version = '1.0.0';
		...
		$this->fields = array(
			array(
				...
				'title'		=> __( 'Default Upload Limit', $this->shortname ),
				'type'		=> 'text',
				'desc'		=> __( 'Default upload limit when activating feature for products.', $this->shortname ),
				'default'	=> 1,
			),
			array(
				'name'		=> 'wc_ptu_uploads_path',
				'title'		=> __( 'Uploads Path', $this->shortname ),
				'type'		=> 'text',
				'desc'		=> __( 'Uploads path for all new uploads, subfolders with the order ID name will be created within this.', $this->shortname ),
				'default'	=> trailingslashit( $uploads['basedir'] ) . 'wc-pay-to-upload',
			),
			array(
				'name'		=> 'wc_ptu_list_type',
				'title'		=> __( 'Allow or Disallow File Types', $this->shortname ),
				'type'		=> 'select',
				'desc'		=> __( 'The file types that are allowed/disallowed by your selection above.', $this->shortname ),
				'values'	=> array(
					'all'	=> __( 'Allow All', $this->shortname ),
					'white'	=> __( 'Whitelist', $this->shortname ),
					'black'	=> __( 'Blacklist', $this->shortname ),
				),
			),
			array(
				'name'		=> 'wc_ptu_file_types',
				'title'		=> __('File Types', $this->shortname ),
				'type'		=> 'text',
				'desc'		=> __('The file types that are allowed/disallowed by your selection above, separate file types by commas.', $this->shortname ),
			),
		);
	...
	function add_meta_boxes() {
		add_meta_box( 'wc_ptu_enable', __( 'Pay to Upload', $this->shortname ), array( &$this, 'product_upload_options'), 'product', 'side' );
		add_meta_box( 'wc_ptu_files', __( 'Uploaded Files', $this->shortname ), array( &$this, 'order_uploaded_files'), 'shop_order', 'side' );
	}
	...
	function woocommerce_init() {
		...
		$this->fields[] = array(
			...
			'title'		=> __('Required Status(es)', $this->shortname ),
			'type'		=> 'multiselect',
			'desc'		=> __('The required order statuses to allow customers to upload files, hold Control or Command to select multiple options.', $this->shortname ),
			'values'	=> $values,
	...
	function order_uploaded_files( $post ) {
		...
		for ($i = 1; $i <= $limit; $i++) {
			...
			if( !empty( $url ) && !empty( $name ) ) {
				printf( __('File #%s: <a href="%s" target="_blank">%s</a>', $this->shortname), $i, $url, $name );
			} else {
				printf( __('File #%s has not been uploaded.', $this->shortname), $i );
			}
			echo '<br/>';
		}
	}
	...
	function product_upload_options( $post ) {
		...
		echo '<p>';
			echo '<label for="_wc_ptu_enable" style="float:left; width:50px;">' . __('Enable', $this->shortname ) . '</label>';
		...
		echo '<p>';
			...
			echo '<label for="_wc_ptu_limit" style="float:left; width:50px;">' . __('Limit', $this->shortname ) . '</label>';
	...
	function check_for_uploadables( $order_id ) {
		...
		if( is_array( $items ) ) {
			foreach( $items as $item ) {
				$limit = (int) get_post_meta( $item['id'], '_wc_ptu_limit', true );
				if( get_post_meta( $item['id'], '_wc_ptu_enable', true ) == 1 && $limit > 0 ) {
					$limits += $limit;
				}
			}
		} else {
			echo wpautop( __( 'Sorry, no files have been uploaded yet.', $this->shortname ) );
		}
		return $limits;
	}
	...
	function uploader( $order_id ) {
		...
		echo '<h2>' . __( 'Upload Files', $this->shortname ) . '</h2>';
		if( isset( $_FILES ) ) {
			...
			foreach( $_FILES as $key => $file ) {
				...
				if( $allow == true ) {
					if( copy( $file['tmp_name'], $filepath ) ) {
						echo '<p class="success">' . __( 'Your file(s) were uploaded successfully.', $this->shortname) . '</p>';
						...
						echo '<p class="error">' . __( 'There was an error in uploading your file(s).', $this->shortname) . '</p>';
					}
				} else {
					echo '<p class="error">' . sprintf( __( 'There %s filetype is not allowed.', $this->shortname), $ext ) . '</p>';
				}
			}
		}
		...
		echo '<form enctype="multipart/form-data" action="" method="POST">';
			...
			if( $upload ) {
				echo '<input type="submit" class="button" value="' . __( 'Upload', $this->shortname ) . '" /><br/>';
				echo '<p>' . sprintf( __( 'Max upload size: %s', $this->shortname ), $upload_mb ) . 'MB</p>';
			}
		echo '</form>';
	}
}